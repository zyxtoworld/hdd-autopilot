...
ARTIFACT="$ARTIFACT_DIR/hdd-autopilot-aarch64-unknown-linux-gnu"
...
print_missing_env_message() {
  echo "Linux aarch64 packaging environment missing: install Rust cargo plus cargo-zigbuild and zig, or build on a Linux aarch64 host with a C compiler." >&2
}
...
check_env() {
  if ! command -v cargo >/dev/null 2>&1; then
    echo "Linux aarch64 packaging environment missing: cargo is not in PATH." >&2
    return 2
  fi
  ...
  if host_is_linux_aarch64 && has_c_compiler; then
    return 0
  fi
...
TMPFILE=$(mktemp "$TMPDIR_VALUE/hdd-autopilot-aarch64-unknown-linux-gnu.XXXXXX")
cleanup() {
  rm -f "$TMPFILE"
}
...
echo "Building hdd-autopilot-aarch64-unknown-linux-gnu..."
...
elif host_is_linux_aarch64 && has_c_compiler; then
...
if [ ! -f "$OUTPUT" ]; then
  echo "Linux aarch64 build failed: expected output not found at $OUTPUT." >&2
  exit 1
fi
