...
print_missing_env_message() {
  if host_is_macos; then
    echo "macOS arm64 打包环境缺失：请安装 Xcode Command Line Tools，或安装 cargo-zigbuild + zig，并确保 xcrun/SDKROOT 可提供 MacOSX.sdk。" >&2
  else
    echo "macOS arm64 打包环境缺失：请安装 cargo-zigbuild、zig，并设置 SDKROOT 或 APPLE_SDK_ROOT 指向可用的 MacOSX.sdk；否则请改到 macOS 主机上构建。" >&2
  fi
}
...
check_env() {
  if ! command -v cargo >/dev/null 2>&1; then
    echo "macOS arm64 打包环境缺失：请先安装 Rust 工具链并确保 cargo 在 PATH 中。" >&2
    return 2
  fi
