#include "app_shared/pool_types.hpp"
...
int StatusResponse::invite_inventory_remaining() const {
    if (pool_stats.has_value()) {
        return pool_stats->invite_unused;
    }
    return inventory_remaining;
}

int StatusResponse::inventory_remaining_for(RewardKind kind) const {
    return kind == RewardKind::Balance ? balance_inventory_remaining() : invite_inventory_remaining();
}

std::string SubmitResponse::preferred_reward_code(RewardKind kind) const {
    if (kind == RewardKind::Balance) {
        if (!balance_code.empty()) {
            return balance_code;
        }
        return invite_code;
    }
...
void from_json(const nlohmann::json& j, SubmitResponse& value) {
    ...
    value.invite_code = j.value("invite_code", "");
    value.balance_code = j.value("balance_code", "");
    const auto reward_code = j.value("reward_code", "");
    if (value.invite_code.empty()) {
        value.invite_code = reward_code;
    }
    if (value.balance_code.empty()) {
        value.balance_code = reward_code;
    }
    if (value.invite_code.empty()) {
        value.invite_code = value.balance_code;
    }
    if (value.balance_code.empty()) {
        value.balance_code = value.invite_code;
    }
    value.ok = j.value("ok", false);
    value.result = j.value("result", "");
