...
void ensure_round_active(const StatusResponse& snapshot, int expected_round_id) {
    ...
    if (snapshot.invite_inventory_remaining() <= 0) {
...
std::string result_label(const std::string& result) {
    ...
    if (result == kResultRoundClosed) {
        return "轮次已关闭";
    }
    if (result == kResultLate) {
        return "提交过晚";
    }
    if (result == "ok" || result == "accepted" || result == "success") {
        return "成功";
    }
    return result.empty() ? "未说明" : result;
}
...
Runner::Runner(Config config)
    : config_(std::move(config)),
      ...
      invite_store_(config_.output_file),
      solver_(config_.device_index) {
}

void Runner::run() {
    std::cout << "开始运行 invite-miner-gpu：这次会用显卡持续尝试邀请码。\n";
...
void Runner::run_auto_tuned() {
    std::cout << "开始运行 invite-miner-gpu 自动调优模式：先测一下这张显卡更适合哪套配置。\n";
    std::cout << "启动前会先校验显卡求解器和难度规则。\n";
    validate_solver_basics(solver_);
...
void Runner::run_cycle(const SolverConfig& solver_config) {
    ...
    std::cout << "当前是第 #" << status.current_round->id
              ...
              << "，还剩 " << status.invite_inventory_remaining() << " 个邀请码。\n";
    ...
    for (;;) {
        ...
        if (now >= next_round_status_poll) {
            try {
                ensure_round_active(pool_client_.get_status_snapshot(), challenge.round_id);
            } catch (...) {
                stop.store(true);
                throw;
            }
        ...
        if (!submit_response.invite_code.empty()) {
            std::cout << "命中了邀请码：" << submit_response.invite_code << "\n";
            invite_store_.save(submit_response.invite_code);
            std::cout << "邀请码已经保存到 " << display_path(invite_store_.path()) << "：" << submit_response.invite_code << "\n";
            pool_client_.reset();
