...
struct PreparedGpuBatch {
    ...
    std::unique_ptr<argon2::opencl::ProgramContext> program_context;
    argon2::Argon2Params params;
    std::unique_ptr<argon2::opencl::ProcessingUnit> unit;
    ...
    PreparedGpuBatch(const Job& job,
        ...
        program_context = std::make_unique<argon2::opencl::ProgramContext>(
            &global,
            std::vector<argon2::opencl::Device>{device},
            argon2::ARGON2_ID,
            argon2::ARGON2_VERSION_13);
        unit = std::make_unique<argon2::opencl::ProcessingUnit>(
            program_context.get(),
            &params,
            &device,
            batch_size,
...
SolveResult Solver::mine_next_batch(const Job& job,
                                    ...
                                    std::atomic<std::int64_t>& attempts) const {
    if (!session.state) {
        throw std::runtime_error("OpenCL solver session is not initialized");
    }
...
std::size_t Solver::estimate_max_batch_size(const Job& job) const {
    ...
    const auto total_global_mem = static_cast<std::size_t>(devices[device_index_]
        .getCLDevice()
    ...
    const auto usable = static_cast<std::size_t>(static_cast<double>(total_global_mem) * 0.5);
