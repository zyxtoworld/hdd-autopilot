...
void ensure_round_active(const StatusResponse& snapshot, int expected_round_id) {
    ...
    if (snapshot.balance_inventory_remaining() <= 0) {
...
Runner::Runner(Config config)
    : config_(std::move(config)),
      ...
      balance_code_store_(config_.output_file),
      solver_(config_.device_index) {
}

void Runner::run() {
    std::cout << "开始运行 balance-miner-gpu：这次会持续尝试余额兑换码。\n";
...
void Runner::run_auto_tuned() {
    std::cout << "开始运行 balance-miner-gpu 自动调优模式：先测一下这张显卡更适合哪套配置。\n";
    std::cout << "启动前会先校验显卡求解器和难度规则。\n";
    validate_solver_basics(solver_);
...
void Runner::run_cycle(const SolverConfig& solver_config) {
    ...
    std::cout << "当前是第 #" << status.current_round->id
              ...
              << "，还剩 " << status.balance_inventory_remaining() << " 个余额兑换码。\n";
    ...
    for (;;) {
        ...
        if (!submit_response.balance_code.empty()) {
            std::cout << "命中了余额兑换码：" << submit_response.balance_code << "\n";
            balance_code_store_.save(submit_response.balance_code);
            std::cout << "余额兑换码已经保存到 " << display_path(balance_code_store_.path()) << "：" << submit_response.balance_code << "\n";
        }
        return;
    }
}
