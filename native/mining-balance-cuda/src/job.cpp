...
Job::Job(JobConfig config)
    : seed_bytes_(config.seed.begin(), config.seed.end()),
      ...
      pass_prefix_(seed_ + ":" + std::to_string(config.round_id) + ":" + config.visitor_id + ":" + std::to_string(config.challenge_id) + ":" + config.session_salt + ":"),
      time_cost_(static_cast<std::uint32_t>(config.time_cost > 0 ? config.time_cost : 1)),
      memory_cost_kb_(static_cast<std::uint32_t>((config.memory_cost_mb > 0 ? config.memory_cost_mb : 1) * 1024)),
...
std::string Job::password_for_nonce(std::uint64_t nonce) const {
    return pass_prefix_ + std::to_string(nonce);
}
