fn main() {
    ...
    let Some(cmake_arch) = macos_cmake_arch(&target_arch) else {
        warn_native_disabled("当前目标架构不是受支持的 macOS 架构，跳过 OpenCL 原生后端构建。");
        return;
    };

    if !host_is_macos() {
        warn_native_disabled("当前不是 macOS 宿主，跳过 OpenCL 原生后端构建。");
        return;
    }

    let Some(dst) = build_native(cmake_arch) else {
        warn_native_disabled("OpenCL 原生后端构建失败，已自动降级为不可用后端。");
        return;
    };
...
fn build_native(cmake_arch: &str) -> Option<std::path::PathBuf> {
    run_with_suppressed_panic_output(|| {
        ...
        config
            ...
            .build_target("mining_opencl_core");
        config.build()
    })
}
