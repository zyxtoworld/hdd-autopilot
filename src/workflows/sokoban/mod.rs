...
use crate::model::{
    AuthCache, AuthConfig, SokobanConfigResponse, SokobanMeResponse, SokobanStartResponse,
};
...
use self::types::{RoundProgress, SokobanDifficultySummary, SokobanRoundSummary};

pub const DONE_MESSAGE: &str = "自动推箱子已完成。";
const TASK_TITLE: &str = "自动推箱子";
...
struct AccountProgressCache {
    summaries: HashMap<String, SokobanDifficultySummary>,
...
fn run_account(
...
) -> io::Result<Vec<SokobanDifficultySummary>> {
    ui::check_cancel(cancel_flag)?;
    ...
    state.lock().unwrap().log.line_fmt(format_args!(
        "账号 {} 已准备好：推箱子包含 {} 难度，最多可同时进行 {} 局，最小操作间隔 {}ms。",
        runtime.email(),
    ...
    for difficulty in difficulties {
        ...
        let mut summary =
            summaries
                .remove(&difficulty)
                .unwrap_or_else(|| SokobanDifficultySummary {
                    email: runtime.email().to_string(),
                    difficulty: difficulty.clone(),
                    ..SokobanDifficultySummary::default()
                });
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的推箱子{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {}，今天还剩 {} 次。",
            summary.email,
...
fn drain_pending_session(
    ...
    me: &SokobanMeResponse,
) -> io::Result<Vec<SokobanRoundSummary>> {
...
struct DifficultyRunPlan {
    difficulty: String,
    summary: SokobanDifficultySummary,
...
fn run_difficulty(
    ...
    progress_cache: &mut AccountProgressCache,
) -> io::Result<SokobanDifficultySummary> {
...
fn start_new_round(
    ...
    runtime: &mut AccountRuntime,
    difficulty: &str,
) -> io::Result<SokobanStartResponse> {
...
fn remaining_from_me(me: &SokobanMeResponse, difficulty: &str) -> i32 {
...
fn merge_round_into_cache(
    ...
    let entry = cache
        ...
        .or_insert_with(|| SokobanDifficultySummary {
...
fn difficulty_order(config: &SokobanConfigResponse) -> Vec<String> {
