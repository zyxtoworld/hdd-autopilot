...
use super::types::{SokobanDifficultySummary, SokobanRoundSummary};
...
pub(super) fn append_round_result(log_dir: &Path, result: &SokobanRoundSummary) -> io::Result<()> {
...
pub(super) fn append_account_summary(
    ...
    email: &str,
    when_unix_ms: i64,
    summaries: &[SokobanDifficultySummary],
) -> io::Result<()> {
...
pub(super) fn log_round_result(log: &ui::TaskLog, result: &SokobanRoundSummary) {
...
fn format_round_result_line(result: &SokobanRoundSummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的推箱子{}难度第 {} 局（{}，对局 {}）已结算：{}",
        ...
        format!("计划步骤 {}", result.planned_steps),
        format!("实际步骤 {}", result.actual_steps),
        format!("耗时 {}", format_duration_ms(result.duration_ms)),
        format_reason_clause(&result.error_message),
    ])
}

fn format_difficulty_summary_line(summary: &SokobanDifficultySummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的推箱子{}难度已跑完：一共玩了 {} 局",
...
fn format_runtime_round_result_line(result: &SokobanRoundSummary) -> String {
...
fn round_status_label(result: &SokobanRoundSummary) -> String {
