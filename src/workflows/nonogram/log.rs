...
use super::types::{NonogramDifficultySummary, NonogramRoundSummary};
...
pub(super) fn append_round_result(log_dir: &Path, result: &NonogramRoundSummary) -> io::Result<()> {
...
pub(super) fn append_account_summary(
    ...
    email: &str,
    when_unix_ms: i64,
    summaries: &[NonogramDifficultySummary],
) -> io::Result<()> {
...
pub(super) fn log_round_result(log: &ui::TaskLog, result: &NonogramRoundSummary) {
...
fn format_round_result_line(result: &NonogramRoundSummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的数织{}难度第 {} 局（{}，对局 {}）已结算：{}",
...
fn format_difficulty_summary_line(summary: &NonogramDifficultySummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的数织{}难度已跑完：一共玩了 {} 局",
...
fn format_runtime_round_result_line(result: &NonogramRoundSummary) -> String {
...
fn round_status_label(result: &NonogramRoundSummary) -> String {
