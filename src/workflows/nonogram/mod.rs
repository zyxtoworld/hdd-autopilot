...
use crate::model::{
    AuthCache, AuthConfig, NonogramConfigResponse, NonogramMeResponse, NonogramStartResponse,
};
...
use self::types::{NonogramDifficultySummary, NonogramRoundSummary, RoundProgress};

pub const DONE_MESSAGE: &str = "自动数织已完成。";
const TASK_TITLE: &str = "自动数织";
...
struct AccountProgressCache {
    summaries: HashMap<String, NonogramDifficultySummary>,
...
fn run_account(
...
) -> io::Result<Vec<NonogramDifficultySummary>> {
    ui::check_cancel(cancel_flag)?;
    ...
    let config = with_auth_retry_api_until_success(
        ...
        runtime,
        "nonogram config",
        |client, auth_token| client.get_nonogram_config(auth_token),
    )?;
    let difficulties = difficulty_order(&config);
    state.lock().unwrap().log.line_fmt(format_args!(
        "账号 {} 已准备好：数织包含 {} 难度，最多可同时进行 {} 局，最小操作间隔 {}ms。",
        runtime.email(),
    ...
    for difficulty in difficulties {
        ...
        let mut summary =
            summaries
                .remove(&difficulty)
                .unwrap_or_else(|| NonogramDifficultySummary {
                    email: runtime.email().to_string(),
                    difficulty: difficulty.clone(),
                    ..NonogramDifficultySummary::default()
                });
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的数织{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {}，今天还剩 {} 次。",
            summary.email,
...
fn drain_pending_session(
    ...
    me: &NonogramMeResponse,
) -> io::Result<Vec<NonogramRoundSummary>> {
...
struct DifficultyRunPlan {
    difficulty: String,
    summary: NonogramDifficultySummary,
...
fn run_difficulty(
    ...
    progress_cache: &mut AccountProgressCache,
) -> io::Result<NonogramDifficultySummary> {
...
fn start_new_round(
    ...
    runtime: &mut AccountRuntime,
    difficulty: &str,
) -> io::Result<NonogramStartResponse> {
    let result = with_auth_retry_api_until_success(
        cancel_flag,
        state,
        runtime,
        "nonogram start",
        |client, auth_token| client.start_nonogram(auth_token, difficulty),
    );
    match result {
        ...
        Ok(start)
            if start.session.session_id <= 0
                || start.session.row_clues.is_empty()
                || start.session.col_clues.is_empty() =>
        {
            Err(io::Error::other(
...
fn remaining_from_me(me: &NonogramMeResponse, difficulty: &str) -> i32 {
...
fn merge_round_into_cache(
    ...
    let entry = cache
        ...
        .or_insert_with(|| NonogramDifficultySummary {
...
fn difficulty_order(config: &NonogramConfigResponse) -> Vec<String> {
