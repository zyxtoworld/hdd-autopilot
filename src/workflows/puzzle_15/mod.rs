...
use crate::model::{
    AuthCache, AuthConfig, Puzzle15ConfigResponse, Puzzle15HistoryResponse, Puzzle15StartResponse,
};
...
pub const DONE_MESSAGE: &str = "自动华容道处理完成。";
            ...
            match run_account(&cancel_flag, &state, &mut runtime) {
                ...
                Err(error) => {
                    state
                        ...
                        .unwrap()
                        .log
                        .line_fmt(format_args!("账号 {} 自动华容道运行失败：{}", email, error));
                    Ok(())
                }
            }
        }));
...
fn run_account(
    ...
    for difficulty in difficulties {
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的华容道{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {:.8}，今天还剩 {} 次。",
            summary.email,
...
fn drain_pending_sessions(
    ...
    history: &Puzzle15HistoryResponse,
...
fn run_difficulty(
    ...
    config: &Puzzle15ConfigResponse,
    difficulty: &str,
    mut summary: Puzzle15DifficultySummary,
...
fn start_new_round(
    ...
    loop {
        ...
        let result = with_auth_retry(state, runtime, |client, auth_token| {
            client.start_puzzle_15(auth_token, difficulty)
        });
        match result {
            Ok(start) if !start.ok => {
                return Err(io::Error::other(
                    "start_failed: 华容道开局接口返回 ok=false",
                ));
            }
            Ok(start) if start.session_id <= 0 || start.board.is_empty() => {
                return Err(io::Error::other(
                    "start_failed: 华容道开局接口返回的数据缺少有效对局",
                ));
            }
...
fn summarize_rounds_by_difficulty(
    email: &str,
    rounds: &[Puzzle15RoundSummary],
) -> HashMap<String, Puzzle15DifficultySummary> {
