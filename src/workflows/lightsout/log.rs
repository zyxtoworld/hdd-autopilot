...
use super::types::{LightsoutDifficultySummary, LightsoutRoundSummary};
...
pub(super) fn append_account_summary(
    ...
    email: &str,
    when_unix_ms: i64,
    summaries: &[LightsoutDifficultySummary],
) -> io::Result<()> {
...
pub(super) fn log_round_result(log: &ui::TaskLog, result: &LightsoutRoundSummary) {
...
fn format_round_result_line(result: &LightsoutRoundSummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的点灯{}难度第 {} 局（{}，对局 {}）已结算：{}",
...
fn format_difficulty_summary_line(summary: &LightsoutDifficultySummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的点灯{}难度已跑完：一共玩了 {} 局",
...
fn format_runtime_round_result_line(result: &LightsoutRoundSummary) -> String {
...
fn round_status_label(result: &LightsoutRoundSummary) -> String {
