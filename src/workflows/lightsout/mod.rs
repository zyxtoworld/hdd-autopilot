...
use crate::model::{
    AuthCache, AuthConfig, LightsoutConfigResponse, LightsoutMeResponse, LightsoutStartResponse,
};
...
use self::types::{LightsoutDifficultySummary, LightsoutRoundSummary, RoundProgress};

pub const DONE_MESSAGE: &str = "自动点灯已完成。";
const TASK_TITLE: &str = "自动点灯";
...
struct AccountProgressCache {
    summaries: HashMap<String, LightsoutDifficultySummary>,
...
fn run_account(
...
) -> io::Result<Vec<LightsoutDifficultySummary>> {
    ui::check_cancel(cancel_flag)?;
    ...
    let config = with_auth_retry_api_until_success(
        ...
        runtime,
        "lightsout config",
        |client, auth_token| client.get_lightsout_config(auth_token),
    )?;
    let difficulties = difficulty_order(&config);
    state.lock().unwrap().log.line_fmt(format_args!(
        "账号 {} 已准备好：点灯包含 {} 难度，最多可同时进行 {} 局，最小操作间隔 {}ms。",
        runtime.email(),
    ...
    for difficulty in difficulties {
        ...
        let mut summary =
            summaries
                .remove(&difficulty)
                .unwrap_or_else(|| LightsoutDifficultySummary {
                    email: runtime.email().to_string(),
                    difficulty: difficulty.clone(),
                    ..LightsoutDifficultySummary::default()
                });
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的点灯{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {}，今天还剩 {} 次。",
            summary.email,
...
fn drain_pending_session(
    ...
    me: &LightsoutMeResponse,
) -> io::Result<Vec<LightsoutRoundSummary>> {
...
struct DifficultyRunPlan {
    difficulty: String,
    summary: LightsoutDifficultySummary,
...
fn run_difficulty(
    ...
    progress_cache: &mut AccountProgressCache,
) -> io::Result<LightsoutDifficultySummary> {
...
fn start_new_round(
    ...
    runtime: &mut AccountRuntime,
    difficulty: &str,
) -> io::Result<LightsoutStartResponse> {
    let result = with_auth_retry_api_until_success(
        cancel_flag,
        state,
        runtime,
        "lightsout start",
        |client, auth_token| client.start_lightsout(auth_token, difficulty),
    );
    match result {
        Ok(start) if !start.ok => Err(io::Error::other(
            "new_round_unavailable: start api returned ok=false",
        )),
        Ok(start) if start.session.session_id <= 0 || start.session.cells.is_empty() => Err(
...
fn remaining_from_me(me: &LightsoutMeResponse, difficulty: &str) -> i32 {
...
fn merge_round_into_cache(
    ...
    let entry = cache
        ...
        .or_insert_with(|| LightsoutDifficultySummary {
...
fn difficulty_order(config: &LightsoutConfigResponse) -> Vec<String> {
