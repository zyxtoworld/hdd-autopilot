...
struct AccountProgressCache {
    summaries: HashMap<String, MemoryDifficultySummary>,
...
pub fn run_batch(
    ...
    print_account_reward_summary(log, "自动记忆翻牌", &reward_summaries);
...
fn run_account(
...
) -> io::Result<Vec<MemoryDifficultySummary>> {
    ui::check_cancel(cancel_flag)?;
    ...
    let config = with_auth_retry_api_until_success(
        ...
        runtime,
        "memory config",
        |client, auth_token| client.get_memory_config(auth_token),
    )?;
    let difficulties = difficulty_order(&config);
    state.lock().unwrap().log.line_fmt(format_args!(
        "账号 {} 已准备好：记忆翻牌包含 {} 难度，最多可同时进行 {} 局，最小操作间隔 {}ms。",
        runtime.email(),
    ...
    for difficulty in difficulties {
        ...
        let mut summary =
            summaries
                .remove(&difficulty)
                .unwrap_or_else(|| MemoryDifficultySummary {
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的记忆翻牌{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {}，今天还剩 {} 次。",
            summary.email,
...
fn remaining_from_me(me: &MemoryMeResponse, difficulty: &str) -> i32 {
...
fn merge_round_into_cache(
    ...
    let entry = cache
        ...
        .or_insert_with(|| MemoryDifficultySummary {
