...
pub(super) fn merge_round_into_summary(
    summary: &mut FlowfreeDifficultySummary,
...
fn status_for_session(session: &FlowfreeSession) -> String {
    if session.won {
        "won".to_string()
...
fn is_finished(session: &FlowfreeSession) -> bool {
