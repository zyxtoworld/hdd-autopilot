...
use crate::model::{
    AuthCache, AuthConfig, FlowfreeConfigResponse, FlowfreeMeResponse, FlowfreeStartResponse,
};
...
pub const DONE_MESSAGE: &str = "自动连线已完成。";
const TASK_TITLE: &str = "自动连线";
...
struct AccountProgressCache {
    summaries: HashMap<String, FlowfreeDifficultySummary>,
...
pub fn run_batch(
    ...
    let state = Arc::new(Mutex::new(BatchState {
        ...
        result_log_dir: resolve_data_file_path("log/flowfree"),
        log: log.clone(),
    }));
...
fn run_account(
...
) -> io::Result<Vec<FlowfreeDifficultySummary>> {
    ui::check_cancel(cancel_flag)?;
    ...
    let config = with_auth_retry_api_until_success(
        ...
        runtime,
        "flowfree config",
        |client, auth_token| client.get_flowfree_config(auth_token),
    )?;
    let difficulties = difficulty_order(&config);
    state.lock().unwrap().log.line_fmt(format_args!(
        "账号 {} 已准备好：连线包含 {} 难度，最多可同时进行 {} 局，最小操作间隔 {}ms。",
        runtime.email(),
    ...
    for difficulty in difficulties {
        ...
        let mut summary =
            summaries
                .remove(&difficulty)
                .unwrap_or_else(|| FlowfreeDifficultySummary {
                    email: runtime.email().to_string(),
                    difficulty: difficulty.clone(),
                    ..FlowfreeDifficultySummary::default()
                });
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的连线{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {}，今天还剩 {} 次。",
            summary.email,
...
fn drain_pending_session(
    ...
    me: &FlowfreeMeResponse,
) -> io::Result<Vec<FlowfreeRoundSummary>> {
...
struct DifficultyRunPlan {
    difficulty: String,
    summary: FlowfreeDifficultySummary,
...
fn run_difficulty(
    ...
    progress_cache: &mut AccountProgressCache,
) -> io::Result<FlowfreeDifficultySummary> {
...
fn start_new_round(
    ...
    runtime: &mut AccountRuntime,
    difficulty: &str,
) -> io::Result<FlowfreeStartResponse> {
    let result = with_auth_retry_api_until_success(
        cancel_flag,
        state,
        runtime,
        "flowfree start",
        |client, auth_token| client.start_flowfree(auth_token, difficulty),
    );
    match result {
        Ok(start) if !start.ok => Err(io::Error::other(
            "new_round_unavailable: start api returned ok=false",
        )),
        Ok(start) if start.session.session_id <= 0 || start.session.endpoints.is_empty() => Err(
            io::Error::other("new_round_unavailable: start api returned invalid session"),
        ),
        Ok(start) => Ok(start),
        Err(error) => Err(error),
    }
...
fn remaining_from_me(me: &FlowfreeMeResponse, difficulty: &str) -> i32 {
...
fn merge_round_into_cache(
    ...
    let entry = cache
        ...
        .or_insert_with(|| FlowfreeDifficultySummary {
...
fn difficulty_order(config: &FlowfreeConfigResponse) -> Vec<String> {
