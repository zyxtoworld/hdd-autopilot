...
pub const DONE_MESSAGE: &str = "自动数独已完成。";
...
struct AccountProgressCache {
    summaries: HashMap<String, SudokuDifficultySummary>,
    ...
    for handle in handles {
        match handle.join() {
            ...
            Err(_) => state
                ...
                .unwrap()
                .log
                .line("自动数独任务异常退出，请查看前面的账号日志定位原因。"),
        }
    }
    print_account_reward_summary(log, "自动数独", &reward_summaries);
...
pub fn run_account_for_limited_free_play_with_log(
    ...
    let email = runtime.email().to_string();
    let summaries =
        run_account_task_until_complete(cancel_flag, &task_log, "自动数独", &email, || {
            run_account(cancel_flag, &state, &mut runtime, &mut progress_cache)
        })?;
...
fn run_account(
    ...
    let config = with_auth_retry_api_until_success(
        ...
        runtime,
        "sudoku config",
        |client, auth_token| client.get_sudoku_config(auth_token),
    )?;
    ...
    for difficulty in difficulties {
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的数独{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {}，今天还剩 {} 次。",
            summary.email,
...
fn start_new_round(
    ...
    let result = with_auth_retry_api_until_success(
        ...
        runtime,
        "sudoku start",
        |client, auth_token| client.start_sudoku(auth_token, difficulty),
    );
    match result {
        Ok(start) if !start.ok => Err(io::Error::other(
            "new_round_unavailable: 数独接口没有返回可玩的新局",
        )),
        Ok(start) if start.session_id <= 0 || start.givens.is_empty() => Err(io::Error::other(
            "new_round_unavailable: 数独接口返回的数据缺少有效对局",
        )),
        Ok(start) => Ok(start),
        Err(error) => Err(error),
    }
