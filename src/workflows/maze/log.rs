...
use super::types::{MazeDifficultySummary, MazeRoundSummary};
...
pub(super) fn append_round_result(log_dir: &Path, result: &MazeRoundSummary) -> io::Result<()> {
...
pub(super) fn append_account_summary(
    ...
    email: &str,
    when_unix_ms: i64,
    summaries: &[MazeDifficultySummary],
) -> io::Result<()> {
...
pub(super) fn log_round_result(log: &ui::TaskLog, result: &MazeRoundSummary) {
...
fn format_round_result_line(result: &MazeRoundSummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的迷宫{}难度第 {} 局（{}，对局 {}）已结算：{}",
...
fn format_difficulty_summary_line(summary: &MazeDifficultySummary) -> String {
    join_clauses(&[
        format!(
            "[{}] 账号 {} 的迷宫{}难度已跑完：一共玩了 {} 局",
...
fn format_runtime_round_result_line(result: &MazeRoundSummary) -> String {
...
fn round_status_label(result: &MazeRoundSummary) -> String {
