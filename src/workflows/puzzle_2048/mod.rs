...
use crate::model::{
    AuthCache, AuthConfig, Puzzle2048ConfigResponse, Puzzle2048HistoryResponse,
...
pub const DONE_MESSAGE: &str = "自动谜题2048处理完成。";
    ...
    for handle in handles {
        match handle.join() {
            ...
            Err(_) => state
                ...
                .unwrap()
                .log
                .line("自动谜题2048线程提前结束：后台线程发生了未处理异常。"),
        }
    }
...
fn run_account(
    ...
    let config = with_auth_retry(state, runtime, |client, auth_token| {
        client.get_puzzle_2048_config(auth_token)
    })?;
    ...
    for difficulty in difficulties {
        ...
        state.lock().unwrap().log.line_fmt(format_args!(
            "账号 {} 的谜题2048{}难度已完成：一共玩了 {} 局，成功 {} 局，失败 {} 局，总收益 {:.8}，今天还剩 {} 次。",
            summary.email,
...
fn drain_pending_sessions(
    ...
    history: &Puzzle2048HistoryResponse,
) -> io::Result<Vec<PuzzleRoundSummary>> {
    let mut rounds = Vec::new();
    let used_today = used_today_by_difficulty(history);
    for item in history.items.iter().filter(|item| is_pending_item(item)) {
        ui::check_cancel(cancel_flag)?;
...
fn run_difficulty(
    ...
    config: &Puzzle2048ConfigResponse,
    difficulty: &str,
    mut summary: PuzzleDifficultySummary,
                ...
                Err(error) if is_active_session_error(&error.to_string()) => {
                    ...
                    *used_today = used_today_by_difficulty(&history);
                    let Some(item) = history
                        .items
                        .iter()
                        .find(|item| is_pending_item(item))
                        .cloned()
                    else {
                        return Err(error);
                    };
...
fn start_new_round(
    ...
    loop {
        ...
        let result = with_auth_retry(state, runtime, |client, auth_token| {
            client.start_puzzle_2048(auth_token, difficulty)
        });
        match result {
            ...
            Ok(start) if start.session_id <= 0 || start.board.is_empty() => {
                return Err(io::Error::other(
                    "start_failed: 谜题2048开局接口返回的数据缺少有效对局",
                ));
            }
...
fn fetch_history(
    state: &Arc<Mutex<BatchState>>,
    runtime: &mut AccountRuntime,
) -> io::Result<Puzzle2048HistoryResponse> {
    with_auth_retry(state, runtime, |client, auth_token| {
        client.get_puzzle_2048_history(auth_token)
    })
}

fn summarize_rounds_by_difficulty(
    email: &str,
    rounds: &[PuzzleRoundSummary],
) -> HashMap<String, PuzzleDifficultySummary> {
